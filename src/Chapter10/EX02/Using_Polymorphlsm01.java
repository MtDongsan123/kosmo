...
//	- 메서드 오버라이딩을 사용해서 부모 클래스의 메서드를 재정의 해서 자식 클래스에서 재사용한다.
//

// 상속을 사용한 다형적 표현
	//	- 상속 관계에서 객체를 생성시 부모 타입으로 선언할 수 있다.
	//	- 부모 타입으로 선언 하면 부모 클래스의 필드와 메서드만 사용 가능함.
	//	- 자식 객체를 생성하면서 부모 타입으로 지정하면 업캐스팅이 자동으로 됨
	//	- 다시 자식 클래스의 필드와 메서드를 사용 하려면 다운캐스팅을 해야 사용 가능
	//	- 다운 캐스팅은 수동으로 적용됨.
...
public class Using_Polymorphlsm01 {

	public static void main(String[] args) {
		//자식에서 객체 생성 후 부모 타입으로는 지정이 가능함.
		// 부모에서 객체 생성 후 자식 타입으로 지정이 불가능.
		...
		A a3 = new C();		//C 클래스는 A 타입이다. ,C는 A이다 (0)
			//a4는 A, B, D 타입을 내포하고 있고, A타입으로 지정
		A a4 = new D();		//D 클래스는 A 타입이다. ,D는 A이다 (0)
		
		//2. B 타입으로 다형적 표현
//오류		B b1 = new A();		//A는 B이다 (X)
			B b2 = new B();		//B는 B이다 (0)
			B b3 = new C();		//C는 B이다 (0)
			B b4 = new D();		//D는 B이다 (0)
