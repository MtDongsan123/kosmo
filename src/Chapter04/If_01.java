	...
	public static void main(String[] args) {
		...
		- if (조건) {
		  ...
		  - if (조건1) {  	  //조건1이 참일때, 실행문1을 실행
		  	...
		  	...
		  	} else { // 위에 모든 조건을 만족하지 않을 때 실행문 10을 실행
		  	  실행문10;
		  	}
		 */
		...
		if (3 < 5) { //조건이 true 이므로 실행문내의 구문 실행됨.
			System.out.println("안녕");
			System.out.println("방가");			
		}
		...
		System.out.println("==================");
		if (5 > 3)
			System.out.println("오늘은 비가옵니다."); //중괄호 생략
			System.out.println("비가 보슬 보슬 옵니다."); //중괄호 밖의 실행구문
		// 중괄호 생략 후 false 일때는 System.out.println("오늘은 비가옵니다."); 실행이 생략됨.
		System.out.println("===================");
		...
		//		- 기본 자료형 (8가지) : 초기값을 할당하지 않으면 오류
				//	boolean:<true,false 참,거짓>, (byte, short, int, long,):정수 (float, double,):실스 char:문자
		...
		if (c >= 90) { 						  //false
			...
			System.out.println("B 학점입니다."); //실행문 실행후 if문을 빠져 나옴
		}else if ( c >= 70 ) {
