...
//class 외부에 올수 있는 것(3가지) : 패키지, 임포트(같은 패키지에 존재하지 않는 클래스), 외부클래스

class A{	//클래스 : 객체를 생성하기 위한 설계도
	//클래스 내부에 올수 있는것(4가지):1. 필드 (클래스에서 선언된변수), 2.생성자, 3.메서드, 4.인너클래스
	//1. 필드
	...
	//		클래스 내부의 다른 생성자가 존재할시 컴파일러가 기본 생성자는 자동으로 만들어 주지 않는다.
	...
	//work2(정수, 정수)
	int work2 (int a, int b) {	//지역변수 : 메서드 블락에서 선언된 변수, Stack에 값이 저장
								// 메서드가 종료되면 없어짐.
...
public class Using_Object {
	...
	public static void main(String[] args) {
		...
		System.out.println("====================");
		//메서드 호출 (work3 (정수, 실수, 문자열)) -리턴 double
